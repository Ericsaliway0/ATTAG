## python gat/_gene_label_prediction_tsne_pertag.py --model_type PERTAG --net_type pathnet --score_threshold 0.4 --learning_rate 0.001 --num_epochs 65 
...
## (kg39) ericsali@erics-MBP-4 gnn_pathways % python gat/_gene_label_prediction_tsne_pertag.py --model_type PERTAG --net_type ppnet --score_threshold 0.9 --learning_rate 0.001 --num_epochs 201
import argparse
import json
import dgl
import torch
...
def choose_model(model_type, in_feats, hidden_feats, out_feats):
    ...
    elif model_type == 'PERTAG':
        return PERTAG(in_feats, hidden_feats, out_feats)
    else:
        raise ValueError("Invalid model type. Choose from ['GraphSAGE', 'GAT', 'EMOGI', 'GCN', 'GIN', 'Chebnet', 'PERTAG'].")
...
def main_oncokb(args):
    ...
    oncokb_path = '__embedding_pathway_gcn_gene_omics_gpu/data/oncokb_1172.txt'
...
def main(args):
    ...
    # AUPRC values for ONGene and OncoKB for each model (rows: models, cols: networks)
    models = ["PERTAG", "GCN", "Chebnet", "GAT", "GraphSAGE", "GIN"]
    networks = ["Protein Network", "Pathway Network", "Gene Network"]

    # AUPRC values for ONGene and OncoKB for each model (rows: models, cols: networks)
    auprc_ongene = [
        [0.96, 0.98, 0.96], # PERTAG
        [0.93, 0.88, 0.87], # GCN
        [0.96, 0.97, 0.95], # Chebnet
        [0.95, 0.93, 0.84], # GAT
        [0.94, 0.92, 0.94], # GraphSAGE
...
def main__(args):
    ...
    # File paths
    drivers_file_path = "__embedding_pathway_gcn_gene/data/796_drivers.txt"
    oncokb_file_path = "__embedding_pathway_gcn_gene_omics_gpu/data/oncokb_1172.txt"
    ongene_file_path = "__embedding_pathway_gcn_gene_omics_gpu/data/oncokb_1172.txt"
    ...
    # File 1: pathnet_PERTAG_all_non_overlapping_threshold0.5_epo{args.num_epochs}
    ...
    save_to_csv(
        os.path.join(output_dir, f"{args.net_type}_{args.model_type}_all_non_overlapping_threshold0.5_epo{args.num_epochs}.csv"),
        all_non_overlapping,
        ["Gene Name", "Score", "Confirmed Gene Set"]
    )

    print("File pathnet_PERTAG_all_non_overlapping_threshold0.5_epo{args.num_epochs} saved.")


    # File 2: pathnet_PERTAG_all_predicted_driver_genes_threshold0.5_epo{args.num_epochs}
...
def main(args):
    ...
    drivers_file_path = "__embedding_pathway_gcn_gene/data/796_drivers.txt"
    oncokb_file_path = "__embedding_pathway_gcn_gene_omics_gpu/data/oncokb_1172.txt"
    ongene_file_path = "__embedding_pathway_gcn_gene_omics_gpu/data/ongene_803.txt"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_type', type=str, choices=['GraphSAGE', 'GAT', 'HGDC', 'EMOGI', 'MTGCN', 'GCN', 'GIN', 'Chebnet', 'PERTAG'], required=True, help="Type of GNN model to use")
