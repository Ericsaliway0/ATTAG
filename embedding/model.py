...
import dgl
from dgl.nn import (
    SAGEConv, GATConv, GINConv, GraphConv, ChebConv, TAGConv
)


##############################
# Base Wrapper for Consistency
##############################
...
# Factory Function
#########################################
def choose_model_(model_type, dim_latent, num_layers, **kwargs):
    model_type = model_type.lower()
    if model_type == "tagcn":
        return TAGCNModel(dim_latent, num_layers, **kwargs)
    elif model_type == "graphsage":
        return GraphSAGEModel(dim_latent, num_layers, **kwargs)
    elif model_type == "gat":
        return GATModel(dim_latent, num_layers, **kwargs)
    elif model_type == "gcn":
        return GCNModel(dim_latent, num_layers, **kwargs)
    elif model_type == "gin":
        return GINModel(dim_latent, num_layers, **kwargs)
    elif model_type == "chebnet":
        return ChebNetModel(dim_latent, num_layers, **kwargs)
    else:
